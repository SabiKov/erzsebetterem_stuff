<?php
	...
	// ----------------------------------------------------------------------------------------------------------------------------------
// Search based on name value	
	if(strcasecmp($dataOut[0], $clientKey) == 0 ) {
		$all_member_query = "SELECT * FROM `members` WHERE `name` LIKE '%".$dataOut[1]."%' ORDER BY `name`";
...
// see the note below regarding CORS. (Cross-Origin Resource Sharing) header.
	...
	//Method calculates the difference between two dates
