<?php
	...
	if($connection === false) {
		$find_member = 'database connection error';
		$profile[] = $find_member;	
	}
	// ----------------------------------------------------------------------------------------------------------------------------------
// Search based on key value	
	if(strcasecmp($dataOut[0], $clientKey) == 0 ) {
		$all_member_query = "SELECT * FROM `members` WHERE `key_number` LIKE '%".$dataOut[1]."%' ORDER BY `name`";
		$all_member_result = mysqli_query($connection, $all_member_query);
		$all_member = '';
				...
				//$all_member .= '<form id="member_' . $row['id'] . '">';
					...
					$all_member .= '<ul class="memberBtn" data-role="listview" data-inset="true">';
						$all_member .= '<li class="ui-field-contain ui-grid-b myBtn">';
							...
							$all_member .= '</div>';
							$all_member .= '<div class="ui-grid-solo">';
								$all_member .= 'Belép? kártya:<input style="background-color:#ede9ce;" readonly name="key_' . $row['id'] . '" id="key_' . $row['id'] . '" value="' . $row['key_number'] . '" type="text" data-mini="true">';
							$all_member .= '</div>';
							$all_member .= '<div class="ui-grid-solo">';
		...
		$all_member = 'database search error';
		$profile[] = $all_member;
	}
	// -------------------------------------------------------------	
...
// see the note below regarding CORS. (Cross-Origin Resource Sharing) header.
	...
	//Methods calculate the difference between two dates
